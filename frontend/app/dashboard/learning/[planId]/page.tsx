...
export default function PlanDetailPage() {
  ...
  if (error) {
    return (
      <div className="rounded-3xl border border-dashed border-rose-200 bg-rose-50/80 p-8 text-center text-rose-500 dark:border-rose-500/40 dark:bg-rose-500/10 dark:text-rose-200">
        <button
          onClick={() => router.push("/dashboard/learning")}
          className="mb-4 inline-flex items-center rounded-full bg-white px-4 py-2 text-sm font-semibold text-rose-500 shadow-sm shadow-rose-100 transition hover:-translate-y-0.5 dark:bg-rose-500/20 dark:text-rose-100 dark:shadow-rose-900/40"
        >
          <ArrowLeft className="mr-2 h-4 w-4" /> Вернуться к обучению
        </button>
        <p className="text-sm font-medium text-rose-500 dark:text-rose-100">{error}</p>
      </div>
    );
  }
  ...
  if (!plan) {
    return (
      <div className="rounded-3xl border border-dashed border-slate-200 bg-white/80 dark:bg-slate-900/70 p-8 text-center text-slate-500 dark:text-slate-400">
        <button
          onClick={() => router.push("/dashboard/learning")}
          className="mb-4 inline-flex items-center rounded-full bg-white px-4 py-2 text-sm font-semibold text-rose-500 shadow-sm shadow-rose-100 transition hover:-translate-y-0.5 dark:bg-rose-500/20 dark:text-rose-200 dark:shadow-rose-900/40"
        >
          <ArrowLeft className="mr-2 h-4 w-4" /> Вернуться к обучению
        </button>
        <p className="text-sm text-slate-600 dark:text-slate-400">План не найден или у него пока нет модулей.</p>
      </div>
    );
  }
  ...
  return (
    <div className="space-y-6 text-slate-900 transition-colors duration-500 dark:text-slate-100">
      <header className="overflow-hidden rounded-3xl border border-white/60 bg-white/75 shadow-xl shadow-slate-200/70 backdrop-blur-xl transition-colors dark:border-slate-800/60 dark:bg-slate-900/70 dark:shadow-slate-950/60">
        <div className="relative gap-6 px-6 py-7 md:flex md:items-center md:justify-between lg:px-10">
          <div className="absolute -top-16 left-8 h-32 w-32 rounded-full bg-sky-100 blur-3xl transition-colors dark:bg-slate-800/50" />
          <div className="absolute -bottom-16 right-6 h-36 w-36 rounded-full bg-indigo-100 blur-3xl transition-colors dark:bg-indigo-900/40" />
          <div className="relative flex items-start gap-4">
            <button
              onClick={() => router.push("/dashboard/learning")}
              className="inline-flex items-center rounded-full border border-slate-200 bg-white/90 px-4 py-2 text-sm font-semibold text-slate-600 shadow-sm shadow-slate-200 transition hover:-translate-y-0.5 dark:border-slate-700/60 dark:bg-slate-900/70 dark:text-slate-300"
            >
              <ArrowLeft className="mr-2 h-4 w-4" /> Назад к планам
            </button>
            <div className="space-y-3">
              <div className="flex items-center gap-2 text-xs font-semibold uppercase tracking-[0.35em] text-slate-400 dark:text-slate-500">
                <BookOpen className="h-4 w-4 text-sky-400" />
                {planCreatedAt ? `Создан ${planCreatedAt}` : "План обучения"}
              </div>
              <h1 className="text-3xl font-semibold text-slate-900 dark:text-slate-100 md:text-4xl">{plan.title}</h1>
              <span
                className={`inline-flex items-center gap-2 rounded-full px-4 py-1 text-xs font-semibold ${
                  planIsCompleted
                    ? "bg-emerald-100 text-emerald-600 dark:bg-emerald-900/30 dark:text-emerald-300"
                    : "bg-sky-100 text-sky-600 dark:bg-slate-800/70 dark:text-slate-200"
                }`}
              >
              ...
              {plan.description && (
                <p className="max-w-2xl text-sm text-slate-500 dark:text-slate-400">{plan.description}</p>
              )}
            </div>
          </div>
          <div className="relative mt-6 flex w-full max-w-sm flex-col gap-3 md:mt-0">
            <div>
              ...
              <div className="mt-1 h-2 rounded-full bg-slate-100 dark:bg-slate-800/70">
                <div
              ...
              <p className="mt-1 text-xs text-slate-500 dark:text-slate-400">
                Завершено уроков: <span className="font-medium text-slate-600 dark:text-slate-200">{completedCount}</span> из {totalLessons}
              </p>
            </div>
            <button
      ...
      <div className="grid gap-6 lg:grid-cols-[320px_minmax(0,1fr)]">
        <aside className="hidden flex-col overflow-hidden rounded-3xl border border-white/60 bg-white/80 p-5 shadow-xl shadow-slate-200/60 backdrop-blur-xl transition-colors dark:border-slate-800/60 dark:bg-slate-900/70 dark:shadow-slate-950/60 lg:flex">
          <h2 className="text-lg font-semibold text-slate-900 dark:text-slate-100">Модули курса</h2>
          <p className="mt-1 text-xs text-slate-500 dark:text-slate-400">Выберите урок, чтобы перейти к материалу.</p>

          <div className="mt-4 flex-1 space-y-5 overflow-y-auto pr-2">
            {plan.modules.length === 0 && (
              <p className="text-sm text-slate-500 dark:text-slate-400">Модули пока не добавлены.</p>
            )}
            ...
            {plan.modules.map((module, moduleIndex) => {
              ...
              return (
                <div
                  key={module.id}
                  className="space-y-2 rounded-2xl bg-slate-50/80 p-4 shadow-sm shadow-slate-200/40 dark:border dark:border-slate-800/60 dark:bg-gradient-to-br dark:from-slate-900/70 dark:via-slate-900/60 dark:to-slate-950/80 dark:shadow-[0_18px_35px_-28px_rgba(15,23,42,0.9)]"
                >
                  <div>
                        ...
                        <li key={lesson.id}>
                          <button
                            ...
                            className={`flex w-full items-center gap-3 rounded-xl px-3 py-2 text-left text-sm transition-all ${
                              isActive
                                ? "border border-sky-200 bg-sky-50 text-sky-700 shadow-sm dark:border-sky-500/40 dark:bg-sky-500/15 dark:text-sky-200"
                          ...
                          >
                            ...
                            <span className="flex-1">
                              <span className="block font-medium text-slate-800 dark:text-slate-200">{lesson.title}</span>
                              <span className="text-xs text-slate-400 dark:text-slate-500">
                                {lesson.type === "practical" ? "Практика" : "Теория"}
                              </span>
                            </span>
                            <ChevronRight className={`h-4 w-4 ${isActive ? "text-sky-500" : "text-slate-300 dark:text-slate-600"}`} />
                          </button>
                        </li>
                      );
                    })}
        ...
        <main className="overflow-hidden rounded-3xl border border-white/60 bg-white/75 p-6 shadow-xl shadow-slate-200/70 backdrop-blur-xl transition-colors dark:border-slate-800/60 dark:bg-slate-900/70 dark:shadow-slate-950/60">
          <div className="mb-4 space-y-3 lg:hidden">
            <h2 className="text-base font-semibold text-slate-900 dark:text-slate-100">Содержание курса</h2>
            {plan.modules.map((module, moduleIndex) => {
              const moduleLessons = module.lessons || [];
              return (
                <div className="rounded-xl border border-slate-200 bg-slate-50/80 p-3 dark:border-slate-700/60 dark:bg-gradient-to-br dark:from-slate-900/70 dark:via-slate-900/60 dark:to-slate-950/80" key={module.id}>
                  <p className="text-sm font-semibold text-slate-800 dark:text-slate-200">
                    Модуль {moduleIndex + 1}. {module.title}
                  </p>
          ...
          {!selectedLesson ? (
            <div className="flex h-full items-center justify-center rounded-2xl border border-dashed border-slate-200 bg-slate-50/80 p-10 text-center text-sm text-slate-500 dark:border-slate-700/60 dark:bg-slate-800/40 dark:text-slate-400">
            ...
            <div className="flex h-full flex-col">
              <div className="flex flex-wrap items-center justify-between gap-4 border-b border-slate-100 pb-4 dark:border-slate-800/60">
                <div>
                  <p className="text-xs uppercase tracking-[0.4em] text-slate-400 dark:text-slate-500">Урок {selectedLessonIndex + 1}</p>
                  <h2 className="text-2xl font-semibold text-slate-900 dark:text-slate-100">{selectedLesson.title}</h2>
                  <p className="mt-1 inline-flex items-center text-sm text-slate-500 dark:text-slate-400">
                ...
                <div className="text-right">
                  <p className="text-xs text-slate-400 dark:text-slate-500">Статус урока</p>
                  <p className="flex items-center justify-end gap-2 text-sm font-medium text-slate-700 dark:text-slate-200">
                    {completedLessons.includes(selectedLesson.id) ? (
                      ...
                      <>
                        <Circle className="h-4 w-4 text-slate-300 dark:text-slate-600" /> В процессе
                      </>
                    )}
                  </p>
                </div>
              ...
              <section className="mt-6 space-y-5">
                <div className="rounded-2xl border border-sky-100/70 bg-sky-50/80 p-5 dark:border-slate-800/60 dark:bg-gradient-to-br dark:from-slate-900/70 dark:via-slate-900/60 dark:to-slate-950/80">
                  <h3 className="text-lg font-semibold text-slate-900 dark:text-slate-100">Теория</h3>
                ...
                <div className="space-y-3">
                  <h3 className="text-lg font-semibold text-slate-900 dark:text-slate-100">Задания</h3>
                  {(!selectedLesson.tasks || selectedLesson.tasks.length === 0) && (
                    <div className="rounded-2xl border border-dashed border-slate-200 bg-slate-50/80 p-4 text-sm text-slate-500 dark:border-slate-700/60 dark:bg-slate-800/40 dark:text-slate-400">
                  ...
                  {selectedLesson.tasks?.map((task, index) => {
                    ...
                    if (Array.isArray(taskOptions) && taskOptions.length > 0) {
                      renderedOptions = (
                        <ul className="ml-5 list-disc space-y-1 text-sm text-slate-600 dark:text-slate-300">
                    ...
                    return (
                      <div key={task.id ?? index} className="rounded-2xl border border-slate-100 bg-white/90 p-4 shadow-sm dark:border-slate-700/60 dark:bg-slate-900/70">
                        <div className="flex items-start justify-between gap-4">
                          <div>
                            <p className="text-sm font-medium text-slate-900 dark:text-slate-100">
                              Вопрос {index + 1}: {task.question}
                            </p>
                            <p className="mt-1 text-xs text-slate-400 dark:text-slate-500">Тип задания: {task.type}</p>
                          </div>
                        </div>
                        ...
                        {task.answer && (
                          <p className="mt-3 rounded-xl bg-slate-50 p-3 text-sm text-slate-500 dark:bg-slate-800/40 dark:text-slate-400">
                            Ответ ментора: <span className="font-medium text-slate-700 dark:text-slate-200">{task.answer}</span>
                          </p>
                        )}
                      </div>
                    );
              ...
              <div className="mt-auto flex flex-col gap-3 border-t border-slate-100 pt-6 md:flex-row md:items-center md:justify-between dark:border-slate-800/60">
                  ...
                  <button
                    ...
                    className={`inline-flex items-center rounded-full px-4 py-2 text-sm font-medium transition ${
                      selectedLessonIndex <= 0
                        ? "cursor-not-allowed border border-slate-100 bg-slate-50 text-slate-300 dark:border-slate-700/40 dark:bg-slate-800/40 dark:text-slate-600"
                        : "border border-slate-200 bg-white text-slate-600 shadow-sm hover:-translate-y-0.5 dark:border-slate-700/60 dark:bg-slate-900/70 dark:text-slate-300 dark:shadow-slate-950/50 dark:hover:bg-slate-800/70"
                    }`}
                  >
                  ...
                  <button
                    ...
                    className={`inline-flex items-center rounded-full px-4 py-2 text-sm font-semibold transition ${
                      selectedLessonIndex >= allLessons.length - 1
                        ? "cursor-not-allowed border border-slate-100 bg-slate-50 text-slate-300 dark:border-slate-700/40 dark:bg-slate-800/40 dark:text-slate-600"
                        : "bg-gradient-to-r from-sky-300 via-indigo-300 to-violet-300 text-slate-900 shadow-lg shadow-sky-100/70 hover:-translate-y-0.5 dark:text-slate-100 dark:shadow-slate-950/50"
                    }`}
                  >
